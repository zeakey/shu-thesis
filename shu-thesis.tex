...
letterpaper=true,
breaklinks=true,
bookmarks=true,%使得生成的pdf文档含有目录索引，方便阅读和定位%By Kuber
colorlinks
]{hyperref}
\usepackage{bibentry}
...
\cfoot{\thepage}  % 添加页脚页码
%\cfoot{\small\thepage}  %%可切换为小一号字体的页码%By Kuber
...
%环境中item编号为固定样式，采用enumerate可变换样式，如[1]->【1】
\usepackage{enumerate} % By Kuber

%字体更换--若有意使用更接近word版宋体的字体，请使用以下两句，但不可使用pdflatex编译
%\usepackage{fontspec} % By Kuber
%\setmainfont{STSongti-SC-Regular}%By Kuber
...
\begin{itemize}
  ...
  \item 常用数学符号的表示方法 \url{http://www.mohu.org/info/symbols/symbols.htm} %By Kuber
  ...
  \item 其他高校Latex论文模板 \url{https://github.com/sjtug/SJTUThesis https://github.com/xueruini/thuthesis}
\end{itemize}
...
%\nobibliography*
\begin{enumerate}
%\begin{enumerate}[【1】] %如有需要可调整item样式为【1】
